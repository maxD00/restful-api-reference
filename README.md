...
|`DELETE`|删除资源.|
|`HEAD`|响应只返回元数据,即包含响应码与`Headers`属性,不返回响应数据,其他与`GET`方法相同.|
...
|`400`|请求时发送的数据,不符合格式要求.如请求体不符合json格式,或url的query参数有误.|
|`401`|未携带有效的身份认证凭证.|
...
**注**:如果仅含message信息无法明确说明,可在响应体中增加code属性.
...
如果需要支持检验某种资源是否已经存在,如:用户`user`注册时包含邮件`email`属性,`email`不允许重复.在注册页面要及时反馈邮箱是否被使用,则可使用下面的请求:

```
HEAD	/users?email=zhangsan@163.com&count=true
